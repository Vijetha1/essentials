...
class FineTuneSGD(Optimizer):
    ...
    def __init__(self, exception_vars, multiplier=0.1, lr=0.01, momentum=0., decay=0.,
                 nesterov=False, **kwargs):
        super(FineTuneSGD, self).__init__(**kwargs)
        with K.name_scope(self.__class__.__name__):
            self.iterations = K.variable(0, dtype='int64', name='iterations')
            self.lr = K.variable(lr, name='lr')
            self.momentum = K.variable(momentum, name='momentum')
    ...
    def get_updates(self, loss, params):
        ...
        self.weights = [self.iterations] + moments
        for p, g, m in zip(params, grads, moments):
            if p not in self.exception_vars:
                multiplied_lr = lr * self.multiplier
            else:
                multiplied_lr = lr
